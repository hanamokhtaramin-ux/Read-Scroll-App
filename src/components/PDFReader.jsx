...
const PDFReader = forwardRef(function PDFReader(
  { fileData, scrollToPage, initialProgress, onProgress, onPageInfo },
  scrollRef
) {
  ...
  useEffect(() => {
    if (!scrollToPage || !pdfLoaded || !containerRef.current || !scrollRef?.current) return
    const canvas = containerRef.current.querySelector(`canvas[data-page="${scrollToPage}"]`)
    if (!canvas) return
    const parentTop = scrollRef.current.getBoundingClientRect().top
    const canvasTop = canvas.getBoundingClientRect().top
    scrollRef.current.scrollTo({
      top: scrollRef.current.scrollTop + canvasTop - parentTop,
      behavior: 'smooth',
    })
  }, [scrollToPage, pdfLoaded])
